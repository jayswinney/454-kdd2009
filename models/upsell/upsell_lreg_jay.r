...
save(upsell_lreg_jay, file = 'models/upsell/upsell_lreg_jay.RData')
...
# ---- churn_lreg_jay_plot ----
# library(ggplot2)
plot_df <- data.frame(cvm = upsell_lreg_jay$cvm, cvup = upsell_lreg_jay$cvup,
                      cvlo = upsell_lreg_jay$cvlo, lambda = upsell_lreg_jay$lambda)
...
ggplot(data = plot_df, aes(x = log(lambda), y = cvm)) +
  ...
  geom_vline(xintercept = log(upsell_lreg_jay$lambda.min),
             linetype = 3, size = 1) +
  geom_vline(xintercept = log(upsell_lreg_jay$lambda.1se),
             linetype = 3, size = 1) +
