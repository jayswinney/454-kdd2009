...
plot(lreg.cv)
```
This plot shows that not all the variables are useful for classification. The regularized and cross validated logistic regression selected a model with 

## number of variables goes here

non-zero variables
<br>

The variables selected by the regularized logistic regression are in the table below with their coefficients.
```{r kable, message=FALSE, warning=FALSE, results= 'asis', echo=FALSE}
cv_coefs <- data.frame(coeficient = coef(lreg.cv)[abs(coef(lreg.cv)) > 1e-5])
row.names(cv_coefs) <- row.names(coef(lreg.cv))[abs(as.vector(coef(lreg.cv))) > 1e-5]
