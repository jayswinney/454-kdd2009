...
```{r kable_churn, message=FALSE, warning=FALSE, results= 'asis', echo=FALSE}
cv_coefs <- data.table(variable = row.names(coef(lreg.cv))[
                          abs(as.vector(coef(lreg.cv))) > 1e-5],
                       coeficient = coef(lreg.cv)[abs(coef(lreg.cv)) > 1e-5])
...
perf <- performance(pred, measure = "tpr", x.measure = "fpr")
ggplot(data.frame(TPR = unlist(perf@y.values),
...
```{r kable_app, message=FALSE, warning=FALSE, results= 'asis', echo=FALSE}
cv_coefs <- data.frame(
...
```{r lreg_app_perf, fig.align='center', message=FALSE, warning=FALSE}
...
```{r dt_app, fig.align='center', message=FALSE, warning=FALSE}
...
```{r rf_app, fig.align='center', message=FALSE, warning=FALSE}
...
```{r rf_app_perf, fig.align='center', message=FALSE, warning=FALSE}
...
```{r lreg_upsell_coefs, fig.align='center', message=FALSE, warning=FALSE, echo = FALSE, eval=FALSE}
